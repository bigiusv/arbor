    ...
    let showCountries rel = do 
        ...
        let showStates rr = do 
            ...
            let showCities rrr = do
                ...
                return  $ ppp <-> mmm <-> nnn
            lll <- list (citiesFromState (identifier mm)) showCities
            ddd <- dropdown (citiesFromState (identifier mm)) bindCities
            dddl <- input "" (value ddd)
            ddc <- dropdown testCustomers bindCustomers
            ddcl <- input "" (value ddc)
            -- return $ mm <|> nn <|> lll
            return $ mm  --- <|> nn
                 <-> (ddd <|> dddl) 
                 <-> (ddc <|> ddcl) 
        ll <- list (statesFromCountry (identifier mod)) showStates
...
dashboard = do
    ...
    custName <- input "customer name: " (string "")
...
testput = do
    ...
    let modbody rel = do iid <-      formInput "id" EmptyValidator (rel ! "id")
                         ...
                         lid <- input "" (string "id")
                         lname <- input "" (string "name")
                         lsurname <- input "" (string "surname")
