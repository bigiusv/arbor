module Language.Bootstrap where
import Language
...
interface = do
    l <- blank -- <?> "Here the user labels his email address"
    h <- home
...
welcome = do 
    inp <- input "Filter sailors" (string "")
    drop <- staticDropdown [string "c1",string "c2",string "c3",string "c4"]
